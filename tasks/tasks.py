...
def build_tasks(code: str, knowledge_base: str = "", pr_author: str = "", max_concurrent: int = 4, requirements: str = "") -> list[Task]:
    ...
    compile_final_review = Task(
        description=(
            ...
            "**FALSE POSITIVE EXAMPLES — DISCARD THESE PATTERNS:**\n"
            "1. A reviewer reports 'Unused variable X in src/web/mock_data.py:144' but the file "
            "src/web/mock_data.py does NOT appear in the PR diff. This is a hallucinated file — DISCARD.\n"
            "2. A reviewer reports 'Potential AttributeError: result.summary.total' but result.summary "
            "is not code from the diff. If you cannot find the exact code in the provided diff, DISCARD.\n"
            "3. A reviewer reports 'KeyError: bbox.x' but no variable named bbox exists in the diff. DISCARD.\n"
            "Rule: If the file path, variable name, or line number cannot be found in the PR diff, "
            "the finding is a hallucination. Always DISCARD it.\n\n"
            "**VERDICT RULES:**\n"
