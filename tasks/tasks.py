...
def build_tasks(code: str, knowledge_base: str = "", pr_author: str = "") -> list[Task]:
    ...
    compile_final_review = Task(
        description=(
            ...
            "## 📝 Review\n\n"
            "Hi @alice, [1-2 natural sentences — overall impression, what caught your eye, balanced tone. Not a dry summary.]\n\n"
            "### ✅ {good_points_header}\n"
            ...
            "If there are NO blocking issues, omit this entire section entirely — do NOT write 'No issues found' or 'Không có issue' inside it. Just skip it.\n\n"
            "### 💡 {suggestions_header} (non-blocking)\n"
            "- [Non-blocking suggestions, can be bullet points or numbered.]\n"
            "- [Include code snippets where helpful.]\n\n"
            "If there are NO suggestions, omit this entire section entirely — do NOT write 'No suggestions' inside it.\n\n"
            "### {conclusion_header}\n"
            ...
            "- FACT-CHECK: Before including ANY suggestion or issue, verify the code you are referencing ACTUALLY EXISTS in the review. "
            "Do NOT describe code patterns, structures, or functions that are not present in the provided diff. "
            "If you cannot find the exact line in the code, do NOT mention it.\n"
            "- SUGGESTION QUALITY GATE: Non-blocking suggestions must meet the same factual accuracy standard as blocking issues. "
            "Every suggestion MUST reference ACTUAL code from the diff — not imagined code, not generic best practices. "
            "If a suggestion is purely about code style preference or theoretical best practice with no concrete code reference, DISCARD it.\n"
